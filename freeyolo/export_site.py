...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .cell .v { font-family:var(--mono); font-weight:700; line-height:1; white-space:nowrap; }
  ...
  .showmore { margin:12px 0 0; font-family:var(--mono); font-size:13px; color:var(--mint); background:var(--panel);
    border:1px solid var(--line); border-radius:8px; padding:9px 14px; cursor:pointer; }
  .showmore:hover { border-color:var(--mint); }
  ...
  @media (max-width:480px){
    .opx,.cost,.curated { font-size:10px; padding:1px 6px; }
    h1 { font-size:clamp(22px,7vw,30px); }
  }
  @media (prefers-reduced-motion:reduce){ .flap,.card:hover::after{animation:none!important} .sec.reveal{opacity:1;transform:none} }
  ...
  @media print {
    ...
    .card, .tnode, .stop { break-inside:avoid; border:1px solid #bbb; background:#fff; color:#000; }
    .card a.t, .tnode .ttitle, .stop .st { color:#000; }
...
// Untagged OR clickbait-y feed items (emoji / #hashtags / junk markup) read as social noise.
...
// Keep the prime "Just added" grid curated — exclude feed slop (#1, round 4).
const fresh=DATA.filter(r=>isNew(r)&&!isNoise(r)).sort((a,b)=>b.found_at.localeCompare(a.found_at)).slice(0,24);
...
if(timelineItems.length) sections.push({id:'board',
  ...
  items:timelineItems.slice(0,40), kind:'timeline'});   // cap so the board can't explode at scale
...
// Pagination: cap each grid's initial DOM so the page scales to 10k+ without a backend.
const PAGE=80; const GRIDS={}; let gridsExpanded=false;
...
function sectionHTML(s,n){
  ...
  // Render only the first PAGE cards (DOM stays bounded at any catalog size); the rest come
  // via "Show more", and an active filter expands every grid so search still sees everything.
  const feat=s.kind==='feat';
  const primary = s.gate ? s.items.filter(r=>!isNoise(r)) : s.items;
  const extra   = s.gate ? s.items.filter(isNoise) : [];
  GRIDS[s.id]={items:primary, feat, shown:Math.min(PAGE,primary.length)};
  const moreBtn = primary.length>PAGE ? `<button class="showmore" data-id="${s.id}">Show ${primary.length-PAGE} more</button>` : '';
  const grid=`<div class="grid ${feat?'feat':''}" id="g-${s.id}">${primary.slice(0,PAGE).map(r=>ticket(r,feat)).join('')}</div>${moreBtn}`;
  const more=extra.length?`<details class="more"><summary>More from the feeds (${extra.length}) — auto-scouted, untagged</summary>
    <div class="grid">${extra.slice(0,PAGE).map(r=>ticket(r,false)).join('')}</div></details>`:'';
...
document.getElementById('topics').innerHTML=topTopics.map(([t,c])=>
  `<button class="top" data-t="${esc(t)}" aria-pressed="false" title="${esc(TOPIC_HUMAN[t]||t)}">${esc(t)}<span class="n">${c}</span></button>`).join('');
...
// While a filter is active, render every grid in full so search sees all items;
// otherwise keep only PAGE per grid in the DOM (bounded at any catalog size).
function expandAllGrids(){ if(gridsExpanded)return;
  for(const id in GRIDS){ const g=GRIDS[id];
    if(g.shown<g.items.length){ document.getElementById('g-'+id).innerHTML=g.items.map(r=>ticket(r,g.feat)).join(''); g.shown=g.items.length; }
    const sm=document.querySelector('.showmore[data-id="'+id+'"]'); if(sm)sm.style.display='none'; }
  gridsExpanded=true; }
function collapseAllGrids(){ if(!gridsExpanded)return;
  for(const id in GRIDS){ const g=GRIDS[id]; g.shown=Math.min(PAGE,g.items.length);
    document.getElementById('g-'+id).innerHTML=g.items.slice(0,g.shown).map(r=>ticket(r,g.feat)).join('');
    const sm=document.querySelector('.showmore[data-id="'+id+'"]'); if(sm){ sm.style.display=''; sm.textContent='Show '+(g.items.length-g.shown)+' more'; } }
  gridsExpanded=false; }
...
function applyFilter(){
  ...
  if(active) expandAllGrids(); else collapseAllGrids();   // bound the DOM unless searching
  document.querySelectorAll('.sec').forEach(sec=>{
    ...
    total+=vis;
    // unfiltered, a paginated grid shows only PAGE cards but its count should be the true total
    const reported=(GRIDS[sec.id] && !active) ? GRIDS[sec.id].items.length : vis;
    sec.style.display=reported?'':'none';
    const ct=sec.querySelector('.ct'); if(ct)ct.textContent=reported;
    const nav=document.querySelector(`nav a[data-sec="${sec.id}"]`);
    if(nav){nav.style.display=reported?'':'none'; const nb=nav.querySelector('.nb'); if(nb)nb.textContent=reported;}
  });
  ...
  if(!total){ const parts=[]; if(term)parts.push(`"${q.value}"`); if(activeBrand)parts.push(activeBrand); if(activeTopic)parts.push(activeTopic); if(onlyFree)parts.push('100% free');
    if(parts.length && MODE!=='archive') empty.textContent=`No departures match ${parts.join(' · ')}. Clear the filters to see the full board.`; }
  syncPressed();
  writeHash();
}
...
document.getElementById('list').addEventListener('click',e=>{
  ...
  if(more){ const id=more.dataset.id, g=GRIDS[id];
    const next=g.items.slice(g.shown, g.shown+PAGE);
    document.getElementById('g-'+id).insertAdjacentHTML('beforeend', next.map(r=>ticket(r,g.feat)).join(''));
    g.shown+=next.length;
    if(g.shown>=g.items.length) more.remove(); else more.textContent='Show '+(g.items.length-g.shown)+' more';
    return; }
...
def _clean_desc(d: str) -> str:
    """Drop scraped image alt-text and truncate at a word boundary (#4)."""
    d = (d or "").strip()
    low = d.lower()
    if _re.match(r"^an? .{0,70}\b(background|collage|hero|image|still|illustration|render|photo|graphic)\b", low):
        return ""
    if len(d) > 200:
        d = d[:200].rsplit(" ", 1)[0].rstrip(",.;:") + "…"
    return d
